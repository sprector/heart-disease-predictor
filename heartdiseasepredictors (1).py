...
# Some variables are categorical, but the histograms still highlight the necessary insights
...
# Fit the best model on the full training set with optimal features
best_model.fit(X_train_rfecv, y_train)

# Making predictions on the test set with optimal features
predictions_grid_search = best_model.predict(X_test_rfecv)

# Print accuracy scores with best parameters
accuracy_grid_search = accuracy_score(y_test, predictions_grid_search)
print("Accuracy on Test Set (Grid Search):", accuracy_grid_search)
...
model = DecisionTreeClassifier(random_state=100, max_depth=3, min_samples_split=5, min_samples_leaf=5)
